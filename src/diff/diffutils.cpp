 ...
 */
DiffPart::DiffPart(Operation _operation, const QString &_text) :
...
QString DiffPart::strOperation(Operation op) {
    switch (op) {
    case INSERT:
        return "INSERT";
    case DELETE:
 ...
 * @return text version
 */
QString DiffPart::toString() const {
 ...
 */
bool DiffPart::operator==(const DiffPart &d) const {
...
QList<DiffPart> diff_match_patch::diff_main(const QString &text1,
                                        const QString &text2) {
...
QList<DiffPart> diff_match_patch::diff_main(const QString &text1,
    ...
    QList<DiffPart> diffs;
    if (text1 == text2) {
        if (!text1.isEmpty()) {
            diffs.append(DiffPart(EQUAL, text1));
        }
        return diffs;
    }
    ...
    if (!commonprefix.isEmpty()) {
        diffs.prepend(DiffPart(EQUAL, commonprefix));
    }
    if (!commonsuffix.isEmpty()) {
        diffs.append(DiffPart(EQUAL, commonsuffix));
    }
...
QList<DiffPart> diff_match_patch::diff_compute(QString text1, QString text2,
                                           bool checklines, clock_t deadline) {
    QList<DiffPart> diffs;

    if (text1.isEmpty()) {
        // Just add some text (speedup).
        diffs.append(DiffPart(INSERT, text2));
        return diffs;
    }

    if (text2.isEmpty()) {
        // Just delete some text (speedup).
        diffs.append(DiffPart(DELETE, text1));
        return diffs;
    }

    {
        ...
        if (i != -1) {
            ...
            diffs.append(DiffPart(op, longtext.left(i)));
            diffs.append(DiffPart(EQUAL, shorttext));
            diffs.append(DiffPart(op, safeMid(longtext, i + shorttext.length())));
            return diffs;
        }
    ...
    if (hm.count() > 0) {
        ...
        const QList<DiffPart> diffs_a = diff_main(text1_a, text2_a,
                                              checklines, deadline);
        const QList<DiffPart> diffs_b = diff_main(text1_b, text2_b,
                                              checklines, deadline);
        // Merge the results.
        diffs = diffs_a;
        diffs.append(DiffPart(EQUAL, mid_common));
        diffs += diffs_b;
        return diffs;
    }
...
QList<DiffPart> diff_match_patch::diff_lineMode(QString text1, QString text2,
                                            clock_t deadline) {
    ...
    QList<DiffPart> diffs = diff_main(text1, text2, false, deadline);
    ...
    diffs.append(DiffPart(EQUAL, ""));
    int count_delete = 0;
    int count_insert = 0;
    QString text_delete = "";
    QString text_insert = "";

    QMutableListIterator<DiffPart> pointer(diffs);
    DiffPart *thisDiff = pointer.hasNext() ? &pointer.next() : nullptr;
...
QList<DiffPart> diff_match_patch::diff_bisect(const QString &text1,
...
QList<DiffPart> diff_match_patch::diff_bisectSplit(const QString &text1,
    ...
    QList<DiffPart> diffs = diff_main(text1a, text2a, false, deadline);
    QList<DiffPart> diffsb = diff_main(text1b, text2b, false, deadline);
...
void diff_match_patch::diff_charsToLines(QList<DiffPart> &diffs,
                                         const QStringList &lineArray) {
    // Qt has no mutable foreach construct.
    QMutableListIterator<DiffPart> i(diffs);
    while (i.hasNext()) {
        DiffPart &diff = i.next();
        QString text;
...
void diff_match_patch::diff_cleanupSemantic(QList<DiffPart> &diffs) {
    if (diffs.isEmpty()) {
        return;
    }
    bool changes = false;
    QStack<DiffPart> equalities;  // Stack of equalities.
    ...
    while (thisDiff != nullptr) {
        if (thisDiff->operation == EQUAL) {
            ...
            if (!lastequality.isNull()
                ...
                pointer.setValue(DiffPart(DELETE, lastequality));
                // Insert a corresponding an insert.
                pointer.insert(DiffPart(INSERT, lastequality));
        ...
        if (prevDiff->operation == DELETE &&
            ...
            if (overlap_length1 >= overlap_length2) {
                if (overlap_length1 >= deletion.length() / 2.0 ||
                    ...
                    pointer.previous();
                    pointer.insert(DiffPart(EQUAL, insertion.left(overlap_length1)));
                    prevDiff->text =
                ...
                if (overlap_length2 >= deletion.length() / 2.0 ||
                    ...
                    pointer.previous();
                    pointer.insert(DiffPart(EQUAL, deletion.left(overlap_length2)));
...
void diff_match_patch::diff_cleanupSemanticLossless(QList<DiffPart> &diffs) {
    ...
    QMutableListIterator<DiffPart> pointer(diffs);
    DiffPart *prevDiff = pointer.hasNext() ? &pointer.next() : nullptr;
    DiffPart *thisDiff = pointer.hasNext() ? &pointer.next() : nullptr;
    DiffPart *nextDiff = pointer.hasNext() ? &pointer.next() : nullptr;
...
void diff_match_patch::diff_cleanupEfficiency(QList<DiffPart> &diffs) {
    if (diffs.isEmpty()) {
        return;
    }
    bool changes = false;
...
void diff_match_patch::diff_cleanupMerge(QList<DiffPart> &diffs) {
    diffs.append(DiffPart(EQUAL, ""));  // Add a dummy entry at the end.
    QMutableListIterator<DiffPart> pointer(diffs);
    int count_delete = 0;
    int count_insert = 0;
    QString text_delete = "";
        ...
        case EQUAL:
            if (count_delete + count_insert > 1) {
                ...
                if (both_types) {
                    ...
                    if (commonlength != 0) {
                        if (pointer.hasPrevious()) {
                            ...
                            pointer.insert(DiffPart(EQUAL, text_insert.left(commonlength)));
                        }
                ...
                if (!text_delete.isEmpty()) {
                    pointer.insert(DiffPart(DELETE, text_delete));
                }
                if (!text_insert.isEmpty()) {
                    pointer.insert(DiffPart(INSERT, text_insert));
                }
...
int diff_match_patch::diff_xIndex(const QList<DiffPart> &diffs, int loc) {
    int chars1 = 0;
    int chars2 = 0;
    int last_chars1 = 0;
    int last_chars2 = 0;
...
QString diff_match_patch::diff_prettyHtml(const QList<DiffPart> &diffs) {
    QString html;
    QString text;
    foreach(DiffPart aDiff, diffs) {
        text = aDiff.text;
...
QString diff_match_patch::diff_text1(const QList<DiffPart> &diffs) {
    QString text;
    foreach(DiffPart aDiff, diffs) {
        if (aDiff.operation != INSERT) {
            text += aDiff.text;
...
QString diff_match_patch::diff_text2(const QList<DiffPart> &diffs) {
    QString text;
    foreach(DiffPart aDiff, diffs) {
        if (aDiff.operation != DELETE) {
            text += aDiff.text;
...
int diff_match_patch::diff_levenshtein(const QList<DiffPart> &diffs) {
    int levenshtein = 0;
    int insertions = 0;
    int deletions = 0;
    foreach(DiffPart aDiff, diffs) {
...
QString diff_match_patch::diff_toDelta(const QList<DiffPart> &diffs) {
    QString text;
    foreach(DiffPart aDiff, diffs) {
        switch (aDiff.operation) {
        case INSERT: {
...
QList<DiffPart> diff_match_patch::diff_fromDelta(const QString &text1,
                                             const QString &delta) {
    QList<DiffPart> diffs;
    ...
    foreach(QString token, tokens) {
        ...
        case '+':
            ...
            diffs.append(DiffPart(INSERT, param));
            break;
        case '-':
            // Fall through.
        case '=': {
...
//                //throw QString("Negative number in diff_fromDelta: %1").arg(param);
            ...
            pointer += n;
            if (token[0] == QChar('=')) {
                diffs.append(DiffPart(EQUAL, text));
            } else {
                diffs.append(DiffPart(DELETE, text));
            }
            break;
        }
        default:
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text) {
    ...
    if (!prefix.isEmpty()) {
        patch.diffs.prepend(DiffPart(EQUAL, prefix));
    }
    // Add the suffix.
    ...
    if (!suffix.isEmpty()) {
        patch.diffs.append(DiffPart(EQUAL, suffix));
    }
...
QList<Patch> diff_match_patch::patch_make(const QString &text1,
    ...
    QList<DiffPart> diffs = diff_main(text1, text2, true);
    if (diffs.size() > 2) {
        diff_cleanupSemantic(diffs);
...
QList<Patch> diff_match_patch::patch_make(const QList<DiffPart> &diffs) {
...
QList<Patch> diff_match_patch::patch_deepCopy(QList<Patch> &patches) {
    ...
    foreach(Patch aPatch, patches) {
        ...
        foreach(DiffPart aDiff, aPatch.diffs) {
            DiffPart diffCopy = DiffPart(aDiff.operation, aDiff.text);
    ...
    foreach(Patch aPatch, patchesCopy) {
        ...
        if (start_loc == -1) {
            ...
            if (text1 == text2) {
                ...
                // indices.
                QList<DiffPart> diffs = diff_main(text1, text2, false);
                if (text1.length() > Match_MaxBits
...
QString diff_match_patch::patch_addPadding(QList<Patch> &patches) {
    ...
    QList<DiffPart> &firstPatchDiffs = firstPatch.diffs;
    if (firstPatchDiffs.empty() || firstPatchDiffs.first().operation != EQUAL) {
        // Add nullPadding equality.
        firstPatchDiffs.prepend(DiffPart(EQUAL, nullPadding));
        ...
        // Grow first equality.
        DiffPart &firstDiff = firstPatchDiffs.first();
    ...
    while (!bigpatch.isNull()) {
        ...
        while (!bigpatch.diffs.isEmpty()) {
            ...
            if (!precontext.isEmpty()) {
                ...
                patch.diffs.append(DiffPart(EQUAL, precontext));
            }
            while (!bigpatch.diffs.isEmpty()
                ...
                if (diff_type == INSERT) {
                    ...
                    empty = false;
                    patch.diffs.append(DiffPart(diff_type, diff_text));
                    bigpatch.diffs.removeFirst();
                } else {
            ...
            if (!postcontext.isEmpty()) {
                ...
                if (!patch.diffs.isEmpty()
                    ...
                    patch.diffs.append(DiffPart(EQUAL, postcontext));
                }
            }
            if (!empty) {
...
QList<Patch> diff_match_patch::patch_fromText(const QString &textline) {
    ...
    while (!text.isEmpty()) {
        ...
        while (!text.isEmpty()) {
            ...
            if (sign == '-') {
                // Deletion.
                patch.diffs.append(DiffPart(DELETE, line));
            } else if (sign == '+') {
                // Insertion.
                patch.diffs.append(DiffPart(INSERT, line));
            } else if (sign == ' ') {
                // Minor equality.
                patch.diffs.append(DiffPart(EQUAL, line));
            } else if (sign == '@') {
                // Start of next patch.
                break;
            } else {
